export const InfiniteBrand = () => {
  return (
    <>
      {/* 
        ВНИМАНИЕ: Обычно, стили с @keyframes определяются в глобальном CSS-файле
        или с использованием CSS-in-JS библиотек (например, Styled Components, Emotion).
        Размещение <style> тега напрямую в компоненте может быть неоптимальным
        для больших приложений из-за возможного повторного рендеринга и управления стилями.
        Это сделано по вашему запросу для демонстрации работы анимации "внутри компонента".
      */}
      <style>
        {`
      ...
      <div className='w-full text-5xl py-8 inline-flex flex-nowrap overflow-hidden [mask-image:_linear-gradient(to_right,transparent_0,_black_128px,_black_calc(100%-200px),transparent_100%)]'>
        {/* Обратите внимание: класс animate-infinite-scroll теперь работает благодаря встроенному <style> тегу */}
        <ul className='flex items-center justify-center md:justify-start sm:[&_li]:mx-8 [&_li]:mx-4 [&_img]:max-w-none' style={{ animation: 'infinite-scroll 25s linear infinite' }}>
          <li>
            <a
              target='_blank'
        ...
        <ul
          ...
          aria-hidden='true'
          style={{ animation: 'infinite-scroll 25s linear infinite' }} // Применяем ту же анимацию
        >
          <li>
            <a
              target='_blank'
